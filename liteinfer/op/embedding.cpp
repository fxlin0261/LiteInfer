...
base::Status EmbeddingLayer::check() const {
    ...
    const int32_t token_size = static_cast<int32_t>(input_tensor.size());
    if (token_size > seq_len_) {
        return base::error::InvalidArgument("The number of input tokens is greater than seq len.");
    }
