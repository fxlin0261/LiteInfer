...
bool PreparePromptThatFits(const model::Llama2Model& model, std::string_view system_prompt,
...
base::Status GenerateAssistantReply(const model::Llama2Model& model,
...
int main(int argc, char* argv[]) {
    if (argc < 3 || argc > 5) {
        LOG(INFO) << "Usage: ./llama2_chat <checkpoint_path> <tokenizer_path> "
    ...
    std::cout << "LiteInfer Llama2 chat\n"
    ...
    while (true) {
        ...
        if (!PreparePromptThatFits(model, system_prompt, &history, trimmed_input, effective_max_new_tokens,
                                   runtime_max_seq_len, &prompt_tokens, &history_trimmed)) {
