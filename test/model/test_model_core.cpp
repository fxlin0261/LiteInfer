...
class FakeEncodeLayer final : public op::EncodeLayerBase {
 public:
  ...
  std::string decode(int32_t token_id) const override { return "tok_" + std::to_string(token_id); }
...
class FakeModel final : public model::Model {
 public:
  ...
  base::Status predict(const tensor::Tensor& input, const tensor::Tensor& pos_tensor, bool is_prompt,
  ...
  base::Status insert_buffer_for_test(model::ModelBufferType buffer_idx, const tensor::Tensor& tensor) {
